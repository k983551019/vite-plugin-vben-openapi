...
class ServiceGenerator {
        ...
        if (schema.type === 'object' && schema.properties) {
            const propertiesList = Object.keys(schema.properties).map((p) => {
                if (schema.properties && schema.properties[p]) {
                    return {
                        key: p,
                        schema: Object.assign(Object.assign({}, schema.properties[p]), { type: getType(schema.properties[p], this.config.namespace) }),
                    };
                }
                return undefined;
            });
    ...
    getTemplate(type) {
        return fs_1.readFileSync(path_1.join(__dirname, '../', 'templates', `${type}.njk`), 'utf8');
    }
    // 获取 TS 类型的属性列表
    ...
    resolveAllOfObject(schemaObject) {
        ...
        let props = [];
        if (allOf.length > 1) {
            props = lodash_1.flatten(allOf.slice(1).map((item) => this.getProps(item)));
        }
        return {
            parent,
            // 属性合并: 根据属性名进行去重
            props: lodash_1.uniqBy(props, 'name'),
        };
    }
    resolveAllOfObjectExpandParent(schemaObject) {
        ...
        let props = [];
        if (allOf.length > 0) {
            props = lodash_1.flatten(allOf.map((item) => {
        ...
        return {
            // 属性合并: 根据属性名进行去重
            props: lodash_1.uniqBy(props.reverse(), 'name').reverse(),
        };
    }
